  ...
  class ProductOperations
    class << self
      def create(spree_product:)
        service = Shopify::ProductExporter.new(spree_product: spree_product)
        service.perform
      end

      def update(spree_product:)
        if spree_product.assembly?
          service = Shopify::BundledProductExporter.new(spree_product: spree_product)
        else
          service = Shopify::ProductUpdater.new(spree_product: spree_product)
        end
